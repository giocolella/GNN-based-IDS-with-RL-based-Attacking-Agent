...
# ===============================
# Modello GAT con 3 layer e metodo di embedding
# ===============================
class GATIDS(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_classes, use_dropout=True, dropout_rate=0.5, heads_first=8):
        super(GATIDS, self).__init__()
        self.use_dropout = use_dropout
        self.dropout_rate = dropout_rate

        # Primo layer: GATConv con multi-head; concat=True restituisce un output di dimensione hidden_dim * heads_first
        self.conv1 = GATConv(input_dim, hidden_dim, heads=heads_first, dropout=dropout_rate, concat=True)
        self.bn1 = nn.BatchNorm1d(hidden_dim * heads_first)

        # Secondo layer: GATConv con heads=1 e concat=False per riportare la dimensione a hidden_dim
        self.conv2 = GATConv(hidden_dim * heads_first, hidden_dim, heads=1, dropout=dropout_rate, concat=False)
        self.bn2 = nn.BatchNorm1d(hidden_dim)

        # Terzo layer: GATConv con heads=1 e concat=False per ottenere output_dim
        self.conv3 = GATConv(hidden_dim, output_dim, heads=1, dropout=dropout_rate, concat=False)
    ...
    def embed(self, x, edge_index):
        """
        Restituisce le embedding dei nodi ottenute dopo i tre layer GAT (prima del layer di output).
        """
        # Primo layer
        x = self.conv1(x, edge_index)
        if x.shape[0] > 1:
...
def load_and_preprocess(csv_path, max_k=10, train_ratio=0.7, val_ratio=0.15, test_ratio=0.15):
    ...
    e suddivide in training, validation e test.
    ...
    # Imputa i valori mancanti usando la media
...
def main():
    ...
    csv_path = ("/Users/mariacaterinadaloia/Documents/GitHub/GNN-based-IDS-with-RL-based-Attacking-Agent/mergedfilteredbig2.csv")  # Modifica il percorso se necessario
    ...
    # - hidden_dim: usato come dimensione base; nel primo layer, con heads_first=8, l'output sarà hidden_dim*8.
    # - output_dim: dimensione intermedia per il terzo layer.
    hidden_dim = 8
    output_dim = 32

    model = GATIDS(input_dim=num_features, hidden_dim=hidden_dim, output_dim=output_dim,
                   num_classes=num_classes, use_dropout=True, dropout_rate=0.5, heads_first=8)
