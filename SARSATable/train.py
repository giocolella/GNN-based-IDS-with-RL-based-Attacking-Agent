...
def pretrain_agent_MSE(agent, features, labels, epochs=1, batch_size=32):
    """
    Pretrains the agent's Q-table using a supervised MSE loss. This function assumes
    that each feature vector is raw (non-normalized) and lets the agent handle discretization.
    The target Q-vector is set to [1, 0] if label==0 and [0, 1] if label==1.
    """
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, total_samples, batch_size):
            ...
            for state, label in zip(batch_features, batch_labels):
                # Let the agent discretize the state
                discrete_state = agent.discretize(state)
                # Retrieve current Q-values (initializing if necessary)
                q_values = agent.get_Q(state)

                # Set target vector based on the label
                ...
                # Update Q-table entry using the learning rate (supervised update)
                agent.Q[discrete_state] = q_values + agent.alpha * error
...
agent = TableSARSAAgent(state_size=state_size, action_size=action_size)
