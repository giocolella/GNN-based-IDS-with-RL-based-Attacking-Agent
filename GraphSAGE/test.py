...
# ===============================
# Modello GraphSAGE con 3 layer e metodo di embedding
# ===============================
class SAGEIDS(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_classes, use_dropout=True, dropout_rate=0.5):
        ...
        # Primo layer: SAGEConv
        self.conv1 = SAGEConv(input_dim, hidden_dim)
        self.bn1 = nn.BatchNorm1d(hidden_dim)

        # Secondo layer: SAGEConv
        self.conv2 = SAGEConv(hidden_dim, hidden_dim)
        self.bn2 = nn.BatchNorm1d(hidden_dim)

        # Terzo layer: SAGEConv
    ...
    def embed(self, x, edge_index):
        """
        Restituisce le embedding dei nodi ottenute dopo i tre layer GraphSAGE (prima del layer di output).
        """
        # Primo layer
        x = self.conv1(x, edge_index)
        if x.shape[0] > 1:
...
def main():
    ...
    hidden_dim = 8      # dimensione del primo layer (e anche del secondo)
    output_dim = 32     # dimensione intermedia del terzo layer

    model = SAGEIDS(input_dim=num_features, hidden_dim=hidden_dim, output_dim=output_dim,
